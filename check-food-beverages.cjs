...
async function checkFoodBeverageImages() {
  ...
  try {
    // First try to list the root folder to see what's available
    console.log('Checking root of product-images bucket...');
    ...
    console.log(`Found ${rootData.length} items in root folder:`);
    rootData.forEach((item, index) => {
      console.log(`${index + 1}. ${item.name} (${item.size || 0} bytes) - ${item.id ? 'Folder' : 'File'}`);
    });
    
    // Now try to list files in food&beverages folder
    console.log('\nChecking food&beverages folder...');
...
async function checkFoodBeverageProducts() {
  ...
  try {
    const { data, error } = await supabase
      ...
      `)
      .or('categories.name.ilike.%food%,categories.name.ilike.%beverage%');
