...
export async function GET(request: NextRequest) {
  try {
    ...
    if (validateTokenFormat(token)) {
      ...
      // Extract user data from token or return default admin user
      let userData;
      
      // For production tokens, extract timestamp and return appropriate user
      if (token.startsWith('prod-jwt-token-')) {
        const tokenParts = token.split('-');
        const timestamp = tokenParts[3];
        
        // Determine user based on token pattern or return default
        userData = {
          success: true,
          message: 'Token valid',
          user: {
