...
function fixImageUrlOnce(url) {
  try {
    ...
    // Decode all parts completely to handle any existing encoding
...
async function finalUrlFix() {
  console.log('🔧 Final URL Fix - Complete Decode and Re-encode\n');
  ...
  try {
    ...
    for (const product of products) {
      try {
        ...
        // Fix all URLs
        const fixedImages = originalImages.map(url => fixImageUrlOnce(url));
