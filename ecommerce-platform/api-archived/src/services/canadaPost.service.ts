import axios, { AxiosInstance, AxiosError, InternalAxiosRequestConfig } from 'axios';import { xml2js, Element, ElementCompact } from 'xml-js';
...
interface CanadaPostRateResponse {
  'price-quotes'?: {
    'price-quote'?: Array<{
      'service-code'?: Array<{ _text: string }>;
      'service-name'?: Array<{ _text: string }>;
      'price-details'?: Array<{ 'due'?: Array<{ _text: string }> }>;
      'service-standard'?: Array<{
        'expected-transit-time'?: Array<{ _text: string }>;
        'expected-delivery-date'?: Array<{ _text: string }>;
      }>;
    }>;
  };
}
...
export class CanadaPostService {
  ...
  constructor(redisClient?: Redis) {
    const env = (process.env.CANADA_POST_ENV || 'development') as 'development' | 'production';
    this.config = {
      env,
  ...
  private handleError = (error: AxiosError): Promise<never> => {
    if (error.response) {
      ...
      try {
        const errorData: CanadaPostErrorResponse = typeof data === 'string' ? xml2js(data, { compact: true }) as CanadaPostErrorResponse : data as CanadaPostErrorResponse;
        ...
        logger.error('Failed to parse Canada Post error response', { error: e });
      }
      ...
      throw new CanadaPostError(
        'No response received from Canada Post',
        'NO_RESPONSE',
        504
      );
    } else {
  ...
  public async getRates(request: ShippingRateRequest): Promise<ShippingRate[]> {
    const cacheKey = `canadapost:rates:${createHash('sha256')
      .update(JSON.stringify(request))
      .digest('hex')}`;

    // Try to get from cache first
    const cached = await this.getFromCache<ShippingRate[]>(cacheKey);
    if (cached) {
      return cached;
    }

    const xml = this.buildRateRequestXml(request);
    const response = await this.client.post<CanadaPostRateResponse>(
      `/rs/ship/price`, 
      xml,
      {
        headers: {
  ...
  private buildRateRequestXml(request: ShippingRateRequest): string {
  ...
  private parseRateResponse(data: any): ShippingRate[] {
    try {
      ...
      if (!Array.isArray(quotes)) {
        throw new CanadaPostError('Invalid response format from Canada Post', 'INVALID_RESPONSE');
      }
      ...
      logger.error('Failed to parse Canada Post rates response', { error, data });
      throw new CanadaPostError(
        'Failed to parse shipping rates',
        'PARSE_ERROR',
        500
      );
    }
  ...
  public async createShipment(request: CreateShipmentRequest): Promise<ShipmentResponse> {
    const xml = this.buildCreateShipmentXml(request);
    const response = await this.client.post(
      `/rs/${this.config.customerNumber}/${this.config.customerNumber}/shipment`,
      xml,
      {
        headers: {
          'Content-Type': 'application/vnd.cpc.ship.rate-v4+xml',
  ...
  private buildCreateShipmentXml(request: CreateShipmentRequest): string {
  ...
  private parseShipmentResponse(data: any): ShipmentResponse {
    try {
      ...
      if (!shipmentInfo) {
        throw new CanadaPostError('Invalid shipment response', 'INVALID_RESPONSE');
      }
      ...
      if (!trackingNumber || !shipmentId || !labelUrl) {
        throw new CanadaPostError('Incomplete shipment response', 'INCOMPLETE_RESPONSE');
      }
      ...
      logger.error('Failed to parse shipment response', { error, data });
      throw new CanadaPostError(
        error instanceof Error ? error.message : 'Failed to parse shipment response',
        'PARSE_ERROR',
        500
      );
    }
  ...
  public async trackPackage(trackingNumber: string): Promise<TrackingResponse> {
    const cacheKey = `canadapost:tracking:${trackingNumber}`;
    
    // Try to get from cache first
    const cached = await this.getFromCache<TrackingResponse>(cacheKey);
    if (cached) {
      return cached;
    }
  ...
  private parseTrackingResponse(data: any): TrackingResponse {
    try {
      ...
      if (!trackingDetail) {
        throw new CanadaPostError('Invalid tracking response', 'INVALID_RESPONSE');
      }
      ...
      logger.error('Failed to parse tracking response', { error, data });
      throw new CanadaPostError(
        'Failed to parse tracking information',
        'PARSE_ERROR',
        500
      );
    }
