...
class TypesenseService {
  ...
  async initializeProductCollection(): Promise<void> {
    const schema = {
      name: 'products',
      fields: [
        { name: 'id', type: 'string' },
  ...
  async getSearchSuggestions(query: string, limit: number = 5): Promise<string[]> {
    try {
      ...
      return searchResults.hits?.map((hit: any) => hit.document.name) || [];
    } catch (error) {
  ...
  private buildSortString(sortOptions: any = {}): string {
...
export const initializeTypesense = async (): Promise<void> => {
  try {
    await typesenseService.initializeProductCollection();
