...
function normalizeImageUrl(url) {
  try {
    ...
    // Decode all parts first to handle any existing encoding
    ...
    const normalizedParts = decodedParts.map((part, idx) => {
      if (idx > bucketIndex) {
...
async function normalizeImageURLsFinal() {
  console.log('🔧 Final Normalization of Image URLs\n');
  ...
  try {
    ...
    for (const product of products) {
      try {
        ...
        // Normalize all URLs
        const normalizedImages = originalImages.map(url => normalizeImageUrl(url));
        ...
        if (hasChanges) {
          ...
          if (originalImages.length > 0 && uniqueImages.length > 0) {
